...
namespace CudafyByExample
{
    ...
    class MatrixCarrier2
    {
        public static ulong CountOfLeaves=1;
        ...
        public static List<Node2> results = new List<Node2>();
        public static List<Node2> toDelete = new List<Node2>();
        ...
        public static void Start()
        {
            ...
            {
                found = false;
                Node2 startNode = new Node2(GetStartStringForFirstNode());
                ...
                Console.WriteLine("------------");
                //Console.WriteLine("количество листьев в дереве: "+CountOfLeaves);
                //Console.ReadLine();
                CountOfLeaves = 1;
            }
        ...
        public static void FillTree(Node2 node)
        {
            //////////threads
            foreach (Node2 item in node.childs)
            {
                if (item.Rang < N)
                {
        ...
        public static void FasterFillOneNode(Node2 startNode)
        {
            foreach (Node2 node in startNode.Parent.childs)
            {
                if (startNode.Parent.childs.IndexOf(node) >= startNode.Parent.childs.IndexOf(startNode))
                {
                    if (CheckStrInTree(startNode, node.Data))
                    {
                        Node2 add = new Node2(node.Data);
                        startNode.Add(add);
                        CountOfLeaves++;
                        if (add.Rang >= L)
        ...
        public static void FillOneNode(Node2 startNode)
        {
        ...
        public static void GetRidOfNodes(Node2 startNode)
        {
                ...
                if (startNode.childs.ElementAt(i).Rang <= N)
                {
                    for (int i2 = i + 1; i2 < startNode.childs.Count; i2++)
                    {
                        if (!CheckArraysOf2Nodes(startNode.childs.ElementAt(i), startNode.childs.ElementAt(i2)))
                        {
                            ...
                            /*Console.WriteLine("getRidOf ---------------------");
                            Console.WriteLine("first node");
                            Node2.PrintTrace(startNode.childs.ElementAt(i));
                            startNode.childs.ElementAt(i).Print();
                            Console.WriteLine("second node");
                            Node2.PrintTrace(startNode.childs.ElementAt(i2));
                            startNode.childs.ElementAt(i2).Print();
                            Console.WriteLine("---------------------");*/
                            //Console.ReadLine();
        ...
        public static bool CheckArraysOf2Nodes(Node2 node, Node2 node2)
        {
            for(int i = 0; i <= L; i++)
            {
        ...
        public static void FindSolution(Node2 node)
        {

            foreach (Node2 child in node.childs)
            {
                if (found)
                    break;
        ...
        public static bool ChechByDifferentColumns(Node2 node)
        {
        ...
        public static bool ChechBySumInColumnsInProcess(Node2 node)
        {
        ...
        public static bool ChechBySumInColumns(Node2 node)
        {
            int[] sumInCol = new int[d];
            for (int i = 0; i < d; i++)
                sumInCol[i] = 0;
        ...
        public static bool CheckStrInTree(Node2 node, string str)
        {
            do
            {
        ...
        public static void PrintToFile(List<Node2> nodes)
        {
            string path = "/Users/1okmon/Desktop/кодовое расстояние для s=2/MatrixCarier2/W_" + S + "_" + L;
            ...
            try
            {
                StreamWriter sw = new StreamWriter("/Users/1okmon/Desktop/кодовое расстояние для s=2/MatrixCarier2/W_" + S + "_" + L + "/N-" + N + "_D-" + D + ".csv");
                sw.WriteLine(N + ";" + "N");
                sw.WriteLine(D + ";" + "D");
                sw.WriteLine(L + ";" + "L");
                sw.WriteLine();
